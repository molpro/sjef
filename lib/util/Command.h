 ...
 *
 * Execution can either by synchronous or asynchronous, but in that case the output and error streams are not available. Standard input is not supported.
 */
class Command {
...
public:
  ...
  std::string operator()(const std::string& command, bool wait = true, std::string directory=".", int verbosity = 0) const ;
...
protected:
  ...
  mutable bp::ipstream m_out;
  mutable bp::ipstream m_err;
  ...
  mutable int m_job_number;
  mutable bp::child m_process;

protected:
  const bool localhost() const {
    return (m_host.empty() || m_host == "localhost" || m_host == "127.0.0.1");
  }
