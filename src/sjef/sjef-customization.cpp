...
void sjef::Project::custom_initialisation() {
    if (m_project_suffix == "molpro") {
        ...
        auto lockfile = molprorc;
        lockfile.replace_filename(".molpro.rc.lock");
...
sjef::Backend sjef::default_backend(const std::string &project_suffix) {
    if (project_suffix == "molpro") {
#ifdef WIN32
        //    auto command = std::string{"'C:/Program Files/Molpro/bin/program_molpro'"};
        auto command = std::string{
            "mpiexec {-n %n:1!MPI size} program_molpro {-M %M!Total memory} {-m %m!Process memory} {-G %G!GA memory}"
        };
#else
