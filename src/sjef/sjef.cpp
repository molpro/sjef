...
void Project::ensure_backend_config_xml() {
  ...
  void Project::ensure_backend_config_yaml() {
  const auto config_file = expand_path(sjef_config_directory() / m_project_suffix / "backends.yaml");
  if (!fs::exists(config_file))
    std::ofstream(config_file) << "" << std::endl;
  {
    auto config_file_ = config_file;
    config_file_ += "-";
    util::Locker locker{fs::path{config_file}.parent_path()};
  ...
  if (false) {
    std::cout << "modified yaml configuration "<<config_file<<std::endl;
    std::string line;
...
void Project::ensure_backend_config_file() {
  ensure_backend_config_xml();
  ensure_backend_config_yaml();
}

void Project::refresh_backends(){
  m_backends.try_emplace(Backend::dummy_name, Backend::local());
  if (!check_backends_xml(m_project_suffix)) {
    ...
    throw runtime_error("sjef xml backend files are invalid");
  }
...
bool sjef::Project::check_backends_xml(const std::string& suffix) const {
